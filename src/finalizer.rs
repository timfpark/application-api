...
pub async fn add(client: Client, name: &str, namespace: &str) -> Result<WorkloadAssignment, Error> {
...
///
/// Note: Does not check for resource's existence for simplicity.
pub async fn delete(client: Client, name: &str, namespace: &str) -> Result<WorkloadAssignment, Error> {
    let api: Api<WorkloadAssignment> = Api::namespaced(client, namespace);
    let finalizer: Value = json!({
        "metadata": {
            "finalizers": null
