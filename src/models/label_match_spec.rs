...
mod tests {
    ...
    fn can_match_exact_spec() {
        ...
        let cluster_spec = LabelMatchSpec { label: "name".to_string(), regex };
    ...
    fn can_not_match_regex() {
        let labels: HashMap<String, String> = [
            ("name".to_string(), "gcp-eastus2-1".to_string())
        ].iter().cloned().collect();

        let cluster = Cluster { name: "gcp-eastus2-1".to_string(), labels};
        let regex = Regex::new("azure-(.)*").unwrap();
