...
pub async fn add_finalizer_record(client: Client, name: &str, namespace: &str) -> Result<WorkloadAssignment, Error> {
    let api: Api<WorkloadAssignment> = Api::namespaced(client, namespace);
...
///
/// Note: Does not check for resource's existence for simplicity.
pub async fn delete_finalizer_record(client: Client, name: &str, namespace: &str) -> Result<WorkloadAssignment, Error> {
    let api: Api<WorkloadAssignment> = Api::namespaced(client, namespace);
    let finalizer: Value = json!({
        "metadata": {
            "finalizers": null
